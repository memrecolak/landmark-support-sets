"""HRNet landmark training with loss masked to a landmark subset.

Forked from `src/training/train.py`. Identical training recipe — same backbone,
same augmentation, same loss, same optimizer/scheduler — except the heatmap
loss is computed only over the indices listed in `data.landmark_subset`.
The model still outputs `data.num_landmarks` channels; non-subset channels
receive no gradient and are ignored at evaluation.

Best-checkpoint selection uses `val/nme_subset` (mean NME over the subset
indices, inter-ocular normalized) rather than the full-98 overall NME.

Usage (from project root):
    python -m src.training.train_subset --config configs/wflw_hrnet_w18_eyesupport_seed42.yaml
"""
...
def main() -> None:
    ...
    landmark_subset = cfg["data"].get("landmark_subset")
    if landmark_subset is None:
        raise SystemExit(
            "config is missing data.landmark_subset — use src.training.train for full-98 training."
        )
    ...
    print(f"loss masked to |S|={len(landmark_subset)} / {cfg['data']['num_landmarks']} landmarks")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    sub_idx = torch.tensor(landmark_subset, dtype=torch.long, device=device)
    ...
    base_regions = cfg["eval"]["regions"]
    eval_regions = {**base_regions, "subset": landmark_subset}
    primary_metric = "subset"
    ...
    for epoch in range(cfg["train"]["epochs"]):
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast("cuda", enabled=amp_enabled):
                pred = model(img)
                loss = criterion(pred.index_select(1, sub_idx), hm.index_select(1, sub_idx))
        ...
        for batch in val_loader:
            ...
            per = nme_per_region(pred_lm, lm_img, iod, eval_regions)
            B = img.size(0)
        ...
        if val[primary_metric] < best_nme:
            best_nme = val[primary_metric]
            torch.save(ckpt, out_dir / "best.pth")
            (out_dir / "best_metrics.json").write_text(json.dumps(val, indent=2))

    writer.close()
    print(f"Done. Best subset NME: {best_nme:.4f}")
